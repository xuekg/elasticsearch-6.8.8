...
public class ZenDiscovery extends AbstractLifecycleComponent implements Discovery, PingContextProvider, IncomingClusterStateListener {
    ...
    public static class NodeRemovalClusterStateTaskExecutor
        ...
        // TODO: 2022/3/31 主节点在探测到节点离线的事件处理中，如果发现当前集群节点数量不足法定人数，则放弃master身份，从而避免产生双主
        @Override
    ...
    /**
     * 选举临时master（该节点票数足够时，才能成为真正的master）
     * 查找当前集群的活跃的master或从候选者选择新的master
     *
     * 1.ping所有节点，获取返回值列表，把本节点单独加到fullPingResponses中
     * 2.构建两个列表 activeMasters：存储集群中活跃的master列表  masterCandidates：存储master候选者列表
     * 3.activeMasters为空的话，从masterCandidates中去选举，可能成功或失败
     * 4.
     * @return
     */
    private DiscoveryNode findMaster() {
        ...
        // filter responses
        //如果节点不具备master资格，则通过ignore_non_master_pings选项忽略它认为的那个master
